...
node combine(node a, node b){
    if(a.mnval <= b.mnval){
        return {a.sum+b.sum,min(a.mn,b.mn),min(a.mnval,b.mnval),a.mnpos};
    }
    return {a.sum+b.sum,min(a.mn,b.mn),min(a.mnval,b.mnval),b.mnpos};
}
 
void build(int idx, int l, int r){
    if(l==r){
        tr[idx] = {arr[l],fsqrt(arr[l]),arr[l],l}; 
    }else{
        int m = (l+r)/2;
        build(idx*2,l,m); 
        build(idx*2+1,m+1,r); 
...
signed main(){
    ...
    while(q--){
        ...
        auto [sum, mn, mnval, pos] = query(l,ll,1,1,n);
        cout << ll-l+1 << " " << sum << "\n";
